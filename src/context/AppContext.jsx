...
export function AppProvider({ children }) {
    ...
    try {
      ...
      if (userInfo && userInfo.id) {
        ...
        try {
          ...
          await db.upsert({ id: userInfo.id, display_name: displayName, avatar_seed: Math.random().toString(36).slice(2), plan: "free", language: "en" });
        } catch { }
    ...
    try {
      ...
      if (pendingRaw) {
        try {
          ...
          const db = await supabase.from("user_profiles", token);
          await db.upsert({ id: userInfo.id, display_name: pending.displayName, avatar_seed: pending.avatar_seed || Math.random().toString(36).slice(2), plan: "free", language: "en" });
  ...
  const loadUserProfile = async (userId, token) => {
    try {
      ...
      const histData = await histDb.select("content_id,progress,last_watched", `&user_id=eq.${userId}&order=last_watched.desc&limit=20`);
      if (Array.isArray(histData)) {
        setWatchHistory(histData.map(r => r.content_id));
        setContinueWatching(histData.filter(r => r.progress > 0 && r.progress < 100).map(r => ({
          id: r.content_id, progress: r.progress,
          lastWatched: r.last_watched ? new Date(r.last_watched).toLocaleDateString() : "Recently",
        })));
      }
  ...
  const dbLogout = async () => {
    if (accessToken) { try { await supabase.signOut(accessToken); } catch { } }
      ...
      handler: async (response) => {
        ...
        if (user && accessToken) {
          try {
            ...
            await db.upsert({ user_id: user.id, plan, status: "active", razorpay_subscription_id: response.razorpay_payment_id, current_period_end: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() });
          } catch { }
        }
      },
    };
